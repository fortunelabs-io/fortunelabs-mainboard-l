...
void ssd1306_display_text_box1(SSD1306_t *dev, int page, int seg, const char *text, int box_width,
...
void ssd1306_display_text_box2(SSD1306_t *dev, int page, int seg, const char *text, int box_width,
                               int text_len, bool invert, int delay) {
    if (page >= dev->_pages)
        return;
    int text_box_pixel = box_width * 8;
...
// by Coert Vonk
void ssd1306_display_text_x3(SSD1306_t *dev, int page, const char *text, int text_len,
                             bool invert) {
    if (page >= dev->_pages)
        return;
    int _text_len = text_len;
    ...
    if (scroll == SCROLL_RIGHT) {
        ...
        for (int page = 0; page < dev->_pages - 1; page++) {
            ...
            for (int seg = _start; seg <= _end; seg++) {
                ...
                wk2 = wk0 | wk1;
                if (seg == 0) {
                    ESP_LOGD(__FUNCTION__, "a page=%d wk0=%02x wk1=%02x wk2=%02x", page, wk0, wk1,
                             wk2);
                }
                if (dev->_flip)
...
void _ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, const uint8_t *bitmap, int width,
    ...
    for (int _height = 0; _height < height; _height++) {
        for (int index = 0; index < _width; index++) {
            for (int srcBits = 7; srcBits >= 0; srcBits--) {
                ...
                ESP_LOGD(__FUNCTION__, "index=%d offset=%d wk1=0x%x page=%d _seg=%d, wk2=%02x",
...
void ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, const uint8_t *bitmap, int width,
                     int height, bool invert) {
...
void _ssd1306_pixel(SSD1306_t *dev, int xpos, int ypos, bool invert) {
    ...
    uint8_t wk1   = 1 << _bits;
    ESP_LOGD(__FUNCTION__, "ypos=%d _page=%d _bits=%d wk0=0x%02x wk1=0x%02x", ypos, _page, _bits,
             wk0, wk1);
    if (invert) {
        wk0 = wk0 & ~wk1;
    } else {
...
void ssd1306_display_rotate_text(SSD1306_t *dev, int seg, const char *text, int text_len,
                                 bool invert) {
    int _text_len = text_len;
    if (_text_len > 8)
        _text_len = 8;
